/**
 * Interactive SVG Phylogenetic Tree Visualizer (D3.js) - Earthy Tones Edition
 * Implements Family Groupings, Interactive Zoom/Pan, High-Contrast Terracotta Links, and Pairwise Inspection.
 */
...
window.TreeViewer = {
  ...
  renderFamilyGroupButtons() {
    ...
    let html = `
      <button onclick="window.TreeViewer.selectFamily('all')" class="family-btn px-3.5 py-1.5 text-xs font-semibold rounded-lg ${this.selectedFamily === 'all' ? 'bg-amber-600 text-stone-950 shadow-md shadow-amber-600/30' : 'bg-stone-900 text-stone-300 hover:bg-stone-800 border border-stone-800'} transition-all">
    ...
    groupings.forEach(g => {
      ...
      html += `
        <button onclick="window.TreeViewer.selectFamily('${g.name}')" class="family-btn px-3.5 py-1.5 text-xs font-semibold rounded-lg ${active ? 'bg-orange-600 text-stone-950 shadow-md shadow-orange-600/30' : 'bg-stone-900 border border-stone-800 text-stone-300 hover:border-amber-500'} transition-all">
    ...
    html += `
      <span class="text-xs text-emerald-400 font-mono bg-emerald-950/40 px-3 py-1.5 rounded-lg border border-emerald-800/40">
  ...
  searchAndHighlight(query) {
    ...
    const svg = d3.select('#treeContainer svg');
    if (!svg.node() || !term) {
      svg.selectAll('.tree-node circle').style('r', d => d.children ? 4.5 : 6.5).style('stroke', '#f97316');
      return;
    }
    ...
    svg.selectAll('.tree-node').each(function(d) {
      ...
      d3.select(this).select('circle')
        .style('r', isMatch ? 10 : (d.children ? 4.5 : 6.5))
        .style('stroke', isMatch ? '#ffffff' : '#f97316')
        .style('stroke-width', isMatch ? '3.5px' : '2.5px');
    });
  },
  ...
  render() {
    ...
    if (this.activeLayout === 'radial') {
      ...
      // Rectangular Phylogram / Cladogram High-Visibility Warm Links
  ...
  onNodeClick(d) {
    ...
    this.updateFamilyDataPanel(`Selected Node: ${name}`, [d]);
  ...
  updateFamilyDataPanel(familyName, matchedNodes = []) {
    ...
    if (!familyName || familyName === 'all') {
      detailBox.innerHTML = `
        <div class="p-4 rounded-xl bg-stone-900/90 border border-amber-900/40 text-xs text-stone-300 flex items-center justify-between">
          <span>🌿 <strong>Interactive Family Explorer:</strong> Select a family button above or click any branch node on the tree to inspect maternal lineage relationships and shared mutations.</span>
          <span class="text-[11px] font-mono text-amber-400 bg-amber-950/60 px-3 py-1 rounded-lg border border-amber-800/60">Viewing All Cohorts</span>
        </div>
      `;
      return;
    }
    ...
    detailBox.innerHTML = `
      <div class="p-5 rounded-2xl bg-gradient-to-r from-stone-900 via-stone-900 to-amber-950/40 border border-orange-500/40 text-xs space-y-3 shadow-2xl">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            <span class="w-3 h-3 rounded-full bg-orange-500 animate-ping"></span>
            <span class="font-extrabold text-orange-300 text-sm font-mono">${familyName} Lineage Breakdown</span>
          </div>
          <span class="px-3 py-1 rounded-lg bg-amber-950 text-amber-300 border border-amber-700 font-mono font-bold">Haplogroup ${haplo}</span>
        </div>
        
        <p class="text-stone-300 leading-relaxed">
          Target family clade isolated on the tree. Displays maternal lineage inheritance, clade position, and divergence metrics across cohort members.
        </p>
        
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 font-mono text-[11px] pt-2 border-t border-stone-800">
          <div class="p-2.5 rounded-lg bg-stone-950/80 border border-stone-800">
            <div class="text-stone-400">Cohort Members</div>
            <div class="text-stone-100 font-bold text-sm">${nodeCount} Sequenced</div>
          </div>
          <div class="p-2.5 rounded-lg bg-stone-950/80 border border-stone-800">
            <div class="text-stone-400">Maternal Clade</div>
            <div class="text-amber-400 font-bold text-sm">${haplo}</div>
          </div>
          <div class="p-2.5 rounded-lg bg-stone-950/80 border border-stone-800">
            <div class="text-stone-400">Tree Zoom Level</div>
            <div class="text-emerald-400 font-bold text-sm">Focused (1.8x)</div>
          </div>
          <div class="p-2.5 rounded-lg bg-stone-950/80 border border-stone-800">
            <div class="text-stone-400">Pairwise Inspection</div>
            <div class="text-orange-400 font-bold text-sm">Click Node to Compare</div>
          </div>
        </div>
      </div>
    `;
